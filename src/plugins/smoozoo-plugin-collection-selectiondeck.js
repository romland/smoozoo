...
add(image) {
    ...
    // --- FIX: Pass the 'image' data when creating the permanent card ---
...
updateDeckLayout(animate = true) {
    ...
    if (uncompressedWidth <= containerContentWidth) {
        ...
        // --- FIX: The offset is now determined purely by the available space. ---
        // We no longer enforce a minimum offset, which was causing the overflow bug.
...
createDeckCard(image) {
    ...
    // --- FIX: Use the passed-in image data to set the source ---
    if (image) {
